...
function updateCartCount() { 
  document.getElementById('cartCount').innerText = orderList.reduce((s,i)=>s+i.quantity,0); 
}
...
function renderProducts(products) {
  ...
  document.querySelectorAll('.add-to-list').forEach(btn => btn.addEventListener('click', (e) => { 
    let id = parseInt(btn.dataset.id), 
        name = btn.dataset.name, 
        price = parseInt(btn.dataset.price), 
        qtyInput = document.getElementById(`qty-${id}`), 
        qty = parseInt(qtyInput.value); 
    if(qty > 0){ 
      let existing = orderList.find(i => i.id === id); 
      if(existing) {
        existing.quantity += qty; 
      } else { 
        orderList.push({id, name, price, quantity: qty}); 
      } 
      saveOrder(); 
      renderOrderSidebar(); 
    } 
...
function renderOrderSidebar() {
  ...
  if(orderList.length === 0) { 
    container.innerHTML = '<p class="empty-order"><i class="fas fa-shopping-cart"></i> No items added yet. Browse products and click "Add to Order List".</p>'; 
    document.getElementById('orderTotal').innerHTML = 'KES 0'; 
    return; 
  }
  
  container.innerHTML = orderList.map(item => `
    <div class="order-item">
      <div class="order-item-details">
        <strong>${item.name}</strong>
        <small>Qty: ${item.quantity} x KES ${item.price.toLocaleString()}</small>
      </div>
...
function filterAndSearch() { 
  let active = document.querySelector('.filter-btn.active').dataset.cat; 
  let search = document.getElementById('searchInput').value.toLowerCase(); 
  let filtered = productsData.filter(p => (active === 'all' || p.category === active) && p.name.toLowerCase().includes(search)); 
...
document.getElementById('whatsappCheckoutBtn').addEventListener('click', () => { 
  ...
  let msg = "Hello Equinox Health Supplies,%0AI would like to order:%0A%0A"; 
  orderList.forEach(i => { 
    msg += `▪ ${i.name} x ${i.quantity} = KES ${(i.price * i.quantity).toLocaleString()}%0A`; 
  }); 
  msg += `%0A📦 TOTAL: KES ${orderList.reduce((s, i) => s + (i.price * i.quantity), 0).toLocaleString()}%0A%0AThank you!`; 
  window.open(`https://wa.me/254706572762?text=${msg}`, '_blank'); 
});

// ============================================
// SIDEBAR UI WITH BODY SCROLL FIX
// ============================================
const trigger = document.getElementById('orderListTrigger');
